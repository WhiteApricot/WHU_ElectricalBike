...
@router.post(
    ...
    description="Run the dispatch optimization stub and return route lines and metrics. / 运行调度优化占位逻辑，返回路线和指标结果。",
)
def optimize_dispatch(payload: DispatchOptimizeRequest) -> DispatchOptimizeResponse:
    return DispatchOptimizeResponse(
        **build_dispatch_optimize_response(period=payload.period, algorithm_type=payload.algorithm_type)
    )
