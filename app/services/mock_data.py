...
HEATMAP_POINTS = {
    PeriodEnum.morning: [
        [30.5396, 114.3644, 0.92],
        [30.5412, 114.3622, 0.78],
        [30.5374, 114.3701, 0.85],
    ],
    PeriodEnum.noon: [
        [30.5401, 114.3662, 0.66],
        [30.5387, 114.3683, 0.71],
        [30.5419, 114.3608, 0.58],
    ],
    PeriodEnum.evening: [
        [30.5379, 114.3711, 0.95],
        [30.5432, 114.3661, 0.74],
        [30.5390, 114.3630, 0.62],
    ],
}
...
def _circle_polygon(lat: float, lng: float, radius_m: float, steps: int = 20) -> list[list[list[float]]]:
    ...
    for index in range(steps):
        ...
        ring.append([
            lng + lng_step * math.cos(angle),
            lat + lat_step * math.sin(angle),
        ])
    ring.append(ring[0])
    return [ring]
...
def _coverage_feature(site_id: str, name: str, lat: float, lng: float, radius_m: float) -> dict[str, Any]:
    return {
        ...
        "properties": {
            "site_id": site_id,
            "name": name,
            "service_radius": radius_m,
        },
        "geometry": {"type": "Polygon", "coordinates": _circle_polygon(lat, lng, radius_m)},
    }


def build_siting_optimize_response(algorithm_type: str, target_sites_count: int, service_radius: float) -> dict[str, Any]:
    ...
    global_metrics = {
        "coverage_ratio": 0.84,
        "average_walking_distance": 108.5,
        "service_radius": service_radius,
        "candidate_sites_count": target_sites_count,
        "algorithm_state": "stub_result",
    }
    return {
        "status": "mock",
...
def build_siting_evaluate_response(current_sites: list[SiteInput]) -> dict[str, Any]:
    sites = current_sites or [
        SiteInput(site_id=site["site_id"], name=site["name"], latitude=site["lat"], longitude=site["lng"], capacity=site["capacity"])
        for site in MOCK_SITE_POINTS[:3]
    ]
    coverage_areas = {
        "type": "FeatureCollection",
        "features": [
            _coverage_feature(
                site.site_id or f"manual-{index + 1}",
                site.name or f"Manual Site {index + 1}",
                site.latitude,
                site.longitude,
                100.0,
            )
    ...
    global_metrics = {
        "coverage_ratio": round(min(0.55 + len(sites) * 0.08, 0.96), 2),
        "average_walking_distance": max(180 - len(sites) * 12, 90),
        "site_balance_index": round(0.75 + len(sites) * 0.03, 2),
        "algorithm_state": "manual_evaluation_stub",
    }
    return {
        "status": "mock",
        "evaluated_sites_count": len(sites),
        "coverage_areas": coverage_areas,
        "global_metrics": global_metrics,
    }
...
def build_dispatch_status(period: PeriodEnum) -> dict[str, Any]:
    period_bias = {
        PeriodEnum.morning: (12, 28, 10),
        PeriodEnum.noon: (18, 18, 4),
        PeriodEnum.evening: (26, 14, -8),
    }[period]
    stations = []
        ...
        stations.append(
            {
                ...
                "inbound": max(delta, 0),
                "outbound": max(-delta, 0),
            }
        )
    return {"status": "mock", "period": period.value, "stations": stations}


def build_dispatch_optimize_response(period: PeriodEnum, algorithm_type: str) -> dict[str, Any]:
    route_features = []
    transfer_plan = []
        ...
        route_features.append(
            {
                ...
                "properties": {
                    ...
                    "from_site": start["site_id"],
                    "to_site": end["site_id"],
                    "transfer_count": 6 + index * 2,
                },
                "geometry": {
                    "type": "LineString",
                    "coordinates": [
        ...
        transfer_plan.append(
            {
                ...
                "to_site": end["site_id"],
                "transfer_count": 6 + index * 2,
                "estimated_duration_min": 9 + index * 3,
            }
        )
    efficiency_metrics = {
        "total_distance_km": 4.8,
        "total_duration_min": 36,
        "estimated_cost": 128.0,
        "supply_demand_balance_rate": 0.88,
        "algorithm_state": "dispatch_stub",
    }
    return {
        "status": "mock",
        "period": period.value,
        "algorithm_type": algorithm_type,
        "dispatch_routes": {"type": "FeatureCollection", "features": route_features},
...
def build_analysis_metrics(scheme_id: str | None) -> dict[str, Any]:
    return {
        ...
        "charts": {
            "coverage_bar": [
                {"label": "manual", "value": 0.73},
                {"label": "optimized", "value": 0.84},
            ],
            "period_line": [
                {"period": "morning", "coverage": 0.79},
                {"period": "noon", "coverage": 0.82},
                {"period": "evening", "coverage": 0.84},
            ],
        },
    }
